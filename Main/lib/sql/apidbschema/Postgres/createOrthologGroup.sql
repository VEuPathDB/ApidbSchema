...
CREATE INDEX ogk_group_ix ON apidb.OrthomclGroupKeyword(ortholog_group_id, keyword, frequency);
...
CREATE INDEX ogd_group_ix ON apidb.OrthomclGroupDomain(ortholog_group_id, frequency, description);
...
CREATE TABLE apidb.OrthomclClade (
  orthomcl_clade_id         NUMERIC(12) NOT NULL,
...
CREATE SEQUENCE apidb.OrthomclClade_sq;

GRANT SELECT ON apidb.OrthomclClade_sq TO gus_r;
GRANT SELECT ON apidb.OrthomclClade_sq TO gus_w;
...
INSERT INTO core.TableInfo
  (table_id, name, table_type, primary_key_column, database_id,
    ...
    other_read, other_write, row_user_id, row_group_id, row_project_id,
    row_alg_invocation_id)
  SELECT nextval('core.tableinfo_sq'), 'OrthomclClade', 'Standard', 'ORTHOMCL_CLADE_ID',
    d.database_id, 0, 0, NULL, NULL, 1, localtimestamp, 1, 1, 1, 1, 1, 1, 1, 1,
    p.project_id, 0
  FROM
       (SELECT MAX(project_id) AS project_id FROM core.ProjectInfo) p,
