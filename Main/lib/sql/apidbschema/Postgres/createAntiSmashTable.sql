...
CREATE TABLE ApiDB.antiSmashClutser (
 ...
 CREATE SEQUENCE ApiDB.antiSmashClutser_sq;
 
 GRANT INSERT, SELECT, UPDATE, DELETE ON ApiDB.antiSmashClutser TO gus_w;
 GRANT SELECT ON ApiDB.antiSmashClutser TO gus_r;
 GRANT SELECT ON ApiDB.antiSmashClutser_sq TO gus_w;
 ...
 INSERT INTO core.TableInfo
   ...
   SELECT NEXTVAL('core.tableinfo_sq'), 'antiSmashClutser', 'Standard', 'antiSmashClutser_id',
     d.database_id, 0, 0, NULL, NULL, 1, localtimestamp, 1, 1, 1, 1, 1, 1, 1, 1, p.project_id, 0
   FROM
        (SELECT MAX(project_id) AS project_id FROM core.ProjectInfo) p,
        (SELECT database_id FROM core.DatabaseInfo WHERE name = 'ApiDB') d
   WHERE 'antiSmashClutser' NOT IN (SELECT name FROM core.TableInfo
