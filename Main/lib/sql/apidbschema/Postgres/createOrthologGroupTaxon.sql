CREATE TABLE apidb.OrthologGroupTaxon (
  ortholog_group_taxon_id NUMERIC(12)  NOT NULL,
  three_letter_abbrev     VARCHAR(20)  NOT NULL,
  number_of_proteins      NUMERIC(12)  NOT NULL,
  number_of_taxa          NUMERIC(12)  NOT NULL,
...
INSERT INTO core.TableInfo
  ...
  SELECT nextval('core.tableinfo_sq'), 'OrthologGroupTaxon', 'Standard', 'ORTHOLOG_GROUP_TAXON_ID',
    d.database_id, 0, 0, NULL, NULL, 1, localtimestamp, 1, 1, 1, 1, 1, 1, 1, 1,
    p.project_id, 0
  FROM
       (SELECT MAX(project_id) AS project_id FROM core.ProjectInfo) p,
       (SELECT database_id FROM core.DatabaseInfo WHERE name = 'ApiDB') d
  WHERE 'OrthologGroupTaxon' NOT IN (SELECT name FROM core.TableInfo
